    ...
    for position in positions:
        ...
        if price_pred > price_curr:
            if position.side.value == "short":
                ...
                market_order = trading_client.submit_order(order_data=MarketOrderRequest(symbol=position.symbol, qty=float(position.qty), side=OrderSide.BUY, time_in_force=TimeInForce.DAY))
        ...
        # acquire a short position if price will drop
            ...
            if position.side.value == "long": 
                close_order = trading_client.close_position(symbol_or_asset_id=position.symbol)
                time.sleep(10)
                market_order = trading_client.submit_order(order_data=MarketOrderRequest(symbol=position.symbol, qty=float(position.qty), side=OrderSide.SELL, time_in_force=TimeInForce.DAY))